...
const steps = [
  {
    key: "en attente de confirmation",
    label: "En attente de confirmation du jeune",
  },
...
const PAYMENT_LABELS = {
  aucun: { label: "Aucun paiement", color: colors.grayLight },
  en_sequestre: { label: "En séquestre", color: colors.accentGold },
  libere: { label: "Libéré au prestataire", color: colors.success },
};

export default function MissionTrackingScreen({ route, navigation }) {
  const { missionId } = route.params;
  const { getMissionById, candidates, getPaymentStatus, getPaymentBreakdown } =
    useMissions();
  const mission = getMissionById(missionId);
  const youth = candidates.find((c) => c.id === mission?.youthId);

  const currentIndex = steps.findIndex((s) => s.key === mission?.status);
  const paymentStatus = getPaymentStatus(mission);
  const paymentInfo = PAYMENT_LABELS[paymentStatus];
  const breakdown = mission?.paymentAmount
    ? getPaymentBreakdown(mission.paymentAmount)
    : null;
      ...
      {breakdown && (
        <View style={styles.paymentCard}>
          <View style={styles.paymentHeader}>
            ...
            >
              <Text style={styles.paymentPillText}>{paymentInfo.label}</Text>
            </View>
          </View>
          <View style={styles.paymentRow}>
            <Text style={styles.paymentLabel}>Montant total</Text>
            <Text style={styles.paymentValue}>
              {breakdown.total.toLocaleString("fr-FR")} F
            </Text>
          </View>
          <View style={styles.paymentRow}>
            <Text style={styles.paymentLabel}>Payé via</Text>
            <Text style={styles.paymentValue}>
              {mission.paymentMethod === "flooz" ? "Flooz" : "Mixx by Moov"}
            </Text>
          </View>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  paymentPillText: { fontSize: 10, color: colors.white, fontWeight: "700" },
  paymentRow: {
    flexDirection: "row",
  ...
  paymentValue: { fontSize: 12, color: colors.dark, fontWeight: "600" },
