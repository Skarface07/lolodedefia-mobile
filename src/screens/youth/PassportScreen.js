...
export default function PassportScreen() {
  ...
  const completedMissions = missions.filter(
    (m) => m.youthId === user?.id && m.status === "terminée"
  );
  const levelIndex = badgeLevels.indexOf(user?.badge ?? "Bronze");
  const qrValue = `LOLODEDEFIA-PRESTATAIRE-${user?.id ?? "inconnu"}`;
          ...
          {badgeLevels.map((lvl, i) => (
            <View key={lvl} style={styles.progressStep}>
              <View
                style={[
                  styles.progressDot,
                  i <= levelIndex && styles.progressDotActive,
                ]}
              />
              <Text
                style={[
                  styles.progressLabel,
                  i <= levelIndex && styles.progressLabelActive,
                ]}
              >
                {lvl}
              </Text>
