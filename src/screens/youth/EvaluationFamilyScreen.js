...
const criteria = ["Clarté des consignes", "Respect et courtoisie", "Ponctualité du paiement", "Recommanderiez-vous ?"];
    ...
    <View style={{ flexDirection: "row" }}>
      {[1, 2, 3, 4, 5].map((n) => (
        <TouchableOpacity key={n} onPress={() => onChange(n)}>
          <Ionicons name={n <= value ? "star" : "star-outline"} size={24} color={colors.accentGold} style={{ marginRight: 4 }} />
        </TouchableOpacity>
      ))}
    </View>
  );
...
export default function EvaluationFamilyScreen({ route, navigation }) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.headerRow}>
        ...
        <Text style={[typography.h2, { marginLeft: spacing.sm }]}>Évaluer la famille</Text>
      </View>
